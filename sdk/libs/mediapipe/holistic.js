(function() {
	...
	*/
	...
	var Ca, Da = "undefined" !== typeof TextDecoder,
		Ea, Fa = "undefined" !== typeof TextEncoder;

	function Ga(a) {
		if (Fa) a = (Ea || (Ea = new TextEncoder)).encode(a);
		else {
			var b = void 0;
			b = void 0 === b ? !1 : b;
	...
	Pa.prototype.toJSON = function() {
		...
		else a = this.g, a = this.g = null == a || "string" === typeof a ? a : Na && a instanceof Uint8Array ? Ja(a) : null;
		return a
	};
	...
	function Sa(a) {
		...
		if (a.constructor === String) return La(a);
		if (a.constructor === Pa) {
			if (null == a.g) var b = Oa || (Oa = new Uint8Array(0));
			else {
				b = Uint8Array;
				var c = a.g;
	...
	function M(a) {
		a.g > a.i && (a.l = !0, ya(Error("Tried to read past the end of the data " + a.g + " > " + a.i)))
	}
	Ta.prototype.j = function() {
		var a = this.h,
			b = a[this.g],
			c = b & 127;
		if (128 > b) return this.g += 1, M(this), c;
		b = a[this.g + 1];
		c |= (b & 127) << 7;
		if (128 > b) return this.g += 2, M(this), c;
		b = a[this.g + 2];
		c |= (b & 127) << 14;
		if (128 > b) return this.g += 3, M(this), c;
		b = a[this.g + 3];
		c |= (b & 127) << 21;
		if (128 > b) return this.g += 4, M(this), c;
		b = a[this.g + 4];
		c |= (b & 15) << 28;
		if (128 > b) return this.g += 5, M(this), c >>> 0;
		this.g += 5;
		if (128 <= a[this.g++] && 128 <= a[this.g++] && 128 <= a[this.g++] && 128 <= a[this.g++] && 128 <= a[this.g++]) return this.l = !0, Aa(), c;
		M(this);
		return c
	};
	...
	function $a(a) {
		...
		b = c >>> 3;
		c &= 7;
		if (!(0 <= c && 5 >= c)) return a.j = !0, za(c, a.l), !1;
		a.i = b;
		a.h = c;
		return !0
	}
		...
		switch (a.h) {
			...
			case 3:
				...
				do {
					...
					if (4 == a.h) {
						a.i != b && (a.j = !0, ya(Error("Unmatched end-group tag")));
						break
					}
					ab(a)
				} while (1);
	...
	function pb(a, b) {
		if (null != a) return Array.isArray(a) || nb(a) ? qb(a, b) : b(a)
	}

	function qb(a, b) {
		if (Array.isArray(a)) {
			for (var c = Array(a.length), d = 0; d < a.length; d++) c[d] = pb(a[d], b);
			kb(a) & 1 && lb(c);
			return c
		}
		c = {};
		for (d in a) c[d] = pb(a[d], b);
		return c
	};
	var rb;
	...
	function O(a, b, c) {
		...
		if (c)
			for (b = 0; b < c.length; b++) a = c[b], a < this.l ? (a += this.j, (d = this.h[a]) ? lb(d) : this.h[a] = sb) : (tb(this), (d = this.i[a]) ? lb(d) : this.i[a] = sb)
	}
	...
	function xb(a, b, c) {
		...
		if (!d) {
			var e = ub(a, c, !1);
			d = [];
			for (var g = Array.isArray(e) ? !!(kb(e) & 2) : !1, f = 0; f < e.length; f++) d[f] = new b(e[f]), g && mb(d[f].h);
	...
	function zb(a, b) {
		if (a = a.o) {
			gb(b, b.g.end());
			for (var c = 0; c < a.length; c++) gb(b, a[c])
		}
	}
	...
	function U(a, b, c) {
		for (; $a(b) && 4 != b.h;) {
			...
			} else d = b, e = a, g = d.l, ab(d), bb(d, e, g)
		}
		return a
	}
	...
	function Cb(a, b, c) {
		...
		try {
			...
			b = a.g, b.h = null, b.m = 0, b.i = 0, b.g = 0, b.l = !1, b.u = !1, a.i = -1, a.h = -1, a.j = !1, 100 > eb.length && eb.push(a)
		}
	}
	...
	function W(a, b, c) {
		...
		if (null != b) {
			...
			var d = b;
			d = (c = 0 > d ? 1 : 0) ? -d : d;
			0 === d ? 0 < 1 / d ? K = L = 0 : (L = 0, K = 2147483648) : isNaN(d) ? (L = 0, K = 2147483647) : 3.4028234663852886E38 < d ? (L = 0, K = (c << 31 | 2139095040) >>> 0) : 1.1754943508222875E-38 > d ? (d = Math.round(d / Math.pow(2, -149)), L = 0, K = (c << 31 | d) >>> 0) : (b = Math.floor(Math.log(d) / Math.LN2), d *= Math.pow(2, -b), d = Math.round(8388608 * d), 16777216 <= d && ++b, L = 0, K = (c << 31 | b + 127 << 23 | d & 8388607) >>> 0);
			c = K;
			N(a, c >>> 0 & 255);
			N(a, c >>> 8 & 255);
			N(a, c >>> 16 & 255);
	...
	function Gb(a, b, c) {
		...
		a = a.h;
		var g;
		if (Da)(g = Ca) || (g = Ca = new TextDecoder("utf-8", {
			fatal: !1
		})), g = g.decode(a.subarray(e, e + d));
		else {
			d = e + d;
			for (var f = [], k = null, h, l, n; e < d;) h = a[e++], 128 > h ? f.push(h) : 224 > h ? e >= d ? f.push(65533) : (l = a[e++], 194 > h || 128 !== (l & 192) ? (e--, f.push(65533)) : f.push((h & 31) << 6 | l & 63)) : 240 > h ? e >= d - 1 ? f.push(65533) : (l = a[e++], 128 !== (l & 192) || 224 === h && 160 > l || 237 === h && 160 <= l || 128 !== ((g = a[e++]) & 192) ? (e--, f.push(65533)) : f.push((h & 15) << 12 | (l & 63) <<
				6 | g & 63)) : 244 >= h ? e >= d - 2 ? f.push(65533) : (l = a[e++], 128 !== (l & 192) || 0 !== (h << 28) + (l - 144) >> 30 || 128 !== ((g = a[e++]) & 192) || 128 !== ((n = a[e++]) & 192) ? (e--, f.push(65533)) : (h = (h & 7) << 18 | (l & 63) << 12 | (g & 63) << 6 | n & 63, h -= 65536, f.push((h >> 10 & 1023) + 55296, (h & 1023) + 56320))) : f.push(65533), 8192 <= f.length && (k = Ba(k, f), f.length = 0);
			g = Ba(k, f)
		}
		Q(b, c, g);
		return !0
	...
	function Qb(a, b) {
		...
		if (null != c && null != c) {
			...
			if (0 <= c) Ya(d, c);
				...
				for (var e = 0; 9 > e; e++) N(d, c & 127 | 128), c >>= 7;
				N(d, 1)
			}
		}
		W(b, a, 2);
	...
	function fc(a, b) {
		...
		if (null != c && null != c) {
			...
			L = e;
			c = K;
			for (g = L; 0 < g || 127 < c;) N(d, c & 127 | 128), c = (c >>> 7 | g << 25) >>> 0, g >>>= 7;
			N(d, c)
		}
		zb(a, b)
	}
	...
	function Ic(a) {
		...
		return H(function(p) {
			switch (p.g) {
				...
				case 2:
					c = p.h;
					if ("object" === typeof window) return Ec("createMediapipeSolutionsWasm", {
						...
						return H(function(m) {
							a.g.graph && a.g.graph.url ? m = F(m,
								Jc(a, a.g.graph.url), 0) : (m.g = 0, m = void 0);
							return m
						})
	...
	function Kc(a) {
		...
		return H(function(l) {
			...
			a.B = {};
			if (a.g.listeners)
				for (g = B(a.g.listeners), f = g.next(); !f.done; f = g.next()) k = f.value, Lc(a, k);
			h = a.j;
			a.j = {};
			a.setOptions(h);
			l.g = 0
						...
						if (k = f.value, h = c.g.inputs[k]) {
							a: {
								...
								switch (h.type) {
									case "video":
										var q = c.m[h.stream];
										q || (q = new yc(c.h, c.D), c.m[h.stream] = q);
									...
									case "detections":
										...
										for (w = 0; w < m.length; ++w) {
											...
											if (t.M)
												for (v = 0; v < t.M.length; ++v) C = q.data, J = C.addClassification, G = w, u = t.M[v], A = new Ob, Q(A, 2, u.score), u.index && Q(A, 1, u.index), u.label && Q(A, 3, u.label), u.displayName && Q(A, 4, u.displayName), u = Bb(A, Qb), J.call(C, G, u)
										}
										q = q.data;
										break a;
									default:
		...
		return H(function(v) {
			switch (v.g) {
				...
				case 2:
					...
					n = f.value;
					r = c[n];
					if ("string" === typeof r) return t = d, w = n, F(v, Oc(a, n, b[r]), 14);
					p = b[r.stream];
					if ("detection_list" === r.type) {
						if (p) {
							var C = p.getRectList();
							for (var J = p.getLandmarksList(),
									G = p.getClassificationsList(), u = [], A = 0; A < C.size(); ++A) {
					...
					if ("proto_list" === r.type) {
						if (p) {
							C = Array(p.size());
							for (J = 0; J < p.size(); J++) C[J] = p.get(J);
							p.delete()
						} else C = [];
						d[n] = C;
						v.g = 7;
	...
	function Lc(a, b) {
		for (var c = b.name || "$", d = [].concat(D(b.wants)), e = new a.h.StringList, g = B(b.wants), f = g.next(); !f.done; f = g.next()) e.push_back(f.value);
				...
				modelComplexity: {
					...
					onChange: function(c) {
						...
						return H(function(k) {
							...
							if (2 != k.g) return F(k, k.h.arrayBuffer(), 2);
							f = k.h;
